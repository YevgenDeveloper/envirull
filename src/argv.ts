...
function processOptionWithEqualValue<T>(
	arg: string,
	...
	return [utils.loadKey(opts, matched[1]), matched[2]];
}
function processOptionAsBooleanValue<T>(
	arg: string,
	...
	return [utils.loadKey(opts, matched[1]), true.toString()];
}
function processOptionAsTypedValue<T>(
	arg: string,
	next: string | undefined,
	opts: data.EnvfullOptions<T>,
	move: () => void
	...
	return [utils.loadKey(opts, matched[1]), next || ""];
}
function processSwitchAsBooleanValue<T>(
	arg: string,
	next: string | undefined,
	opts: data.EnvfullOptions<T>
): [string, string] | null {
...
function processSwitchAsTypedValue<T>(
	arg: string,
	next: string | undefined,
	opts: data.EnvfullOptions<T>,
	move: () => void
